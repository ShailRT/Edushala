...
from .models import AbroadModel, CollegeModel, CourseModel, Navbar
...
def college_page(request, college_slug):
    ...
    related_col = [CollegeModel.objects.all()[:4]]
...
def abroad_page(request, abroad_slug):
    abroad = get_object_or_404(AbroadModel, slug=abroad_slug)
    fees_table = re.split('-|\r\n', abroad.fees_table)
    ct = {}
    current = ''
    for i in range(len(fees_table)):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('abroad-page', abroad_slug=abroad_slug)
    else:
        form = LeadForm()

    # c_key = [f.name for f in CourseModel._meta.get_fields()]
    return render(request, 'study-abroad.html', {'abroad': abroad, 'form': form, 'ct': ct})
